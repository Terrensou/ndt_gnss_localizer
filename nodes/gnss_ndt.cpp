...
NdtLocalizer::NdtLocalizer(ros::NodeHandle &nh, ros::NodeHandle &private_nh) : nh_(nh), private_nh_(private_nh),
    ...
    path_enu_pub_ = nh_.advertise<nav_msgs::Path>("/integrated_nav/Path_ENU", 10, true);
    navsatfix_pub_ = nh_.advertise<sensor_msgs::NavSatFix>("/integrated_nav/Navsatfix", 10, true);
    ...
    save_path = nh.advertiseService("ndt_localizer/save_odom_path",
                                                    &NdtLocalizer::save_path2tum, this);
...
bool NdtLocalizer::save_path2tum(std_srvs::Empty::Request &req,
                   std_srvs::Empty::Response &res){
    ...
    std::ofstream fout;
    fout.open(save_path_dir_+std::to_string(ros::Time::now().toSec()));
